...
if [ -f .env ]; then
  ...
  echo ".env file not found. Please create one with the required STRIPE_WEBHOOK_SECRET and STRIPE_API_KEY."
  exit 1
fi

# Check if STRIPE_WEBHOOK_SECRET is set
if [ -z "$STRIPE_WEBHOOK_SECRET" ]; then
  echo "STRIPE_WEBHOOK_SECRET is not set. Please provide it in the .env file."
  exit 1
fi
